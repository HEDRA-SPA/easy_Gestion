...
Validacion despues de brakes en las funciones en acciones.js y consultas.js
...
- [SI] **Sincronía de Edición:**
    - [SI] Al cambiar el monto de un pago, el `saldo_restante_periodo` se actualizó en TODOS los documentos de pago de ese mismo mes. (No exactamente en todos, ya uqe el saldo restante esta bien que sea disntinto por que fue cambiando mientras mas se fue pagando, lo importante es uque es coherente)
...
- [ ] **Colección MANTENIMIENTO:**
    ...
    - [A TRATAR] Si la unidad estaba ocupada, `afecta_inquilino` es `true` y guardó el `id_inquilino_afectado`.
...
- [SI] **Validación de Restricciones:**
    - [SI] Si existen pagos registrados:
        ...
        - [SI] Se mostró error descriptivo: "NO_SE_PUEDE_MODIFICAR_FECHAS" o "NO_SE_PUEDE_MODIFICAR_DEPO TO".
...
- [SI] **Cambios de Renta:**
    - [SI] Si hay pagos registrados Y cambió la renta:
        ...
        - [SI] Periodos con estatus `"pagado"` NO se modificaron.
    - [SI] Si NO hay pagos:
...
- [ ] **Estados Válidos:**
    ...
    - [ ] `CONTRATO.estatus` es uno de: "activo", "finalizado", "renovado". (checar lo de las fechas, si es proximo o esta actuvo de verdad desde ya)
...
1. Agregar una seccion desplegable de las unidades particulares de cada propiedad para que se puedan eliminar.
 Agregar una validacion para que si se desea ingresar un inquilino que ya haya tenido contrato antes se agregue como reactivacion 
 Que no se puedan realizar registros de contratos solapados en las mismas fechas de otro contrato.
 Pendientes: pensar como estructurar el mantenimiento y la affeccion del inquilino. 
 En relacion a la UI, me hace falta ver que si meto un contrato de arriendo por un mes, aparezca en orden con otros contratos de otros inquilinos pero que iniciaran despues.

 Pendiente examinar el por que el inquilino de CH-1 esta recibiendo lecturas como si existieran pagos aunque no existan.
 Se detecta que sucedieron contratos con fechas solapadas, por eso la consfusion en la base de datos. Se deberia confirmar si ya existe uan validacion para esto.
 Para probar este punto, en CH-5 se pagan todos los periodos de duracion del contrato (2026-01, 2026-06). No hay validacion, ese es el problema.
 Parte corregida pero hace falta que se aglopen todos los nuevos inicios en los usuarios.
